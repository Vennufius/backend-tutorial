...
router.post('/', async (req, res, next) => {
  const { name } = req.body;
  if(!name) {
    const error = new Error('Name not defined.');
    res.status(422);
    return next(error);
  }
...
router.delete('/:id', async (req, res, next) => {
  ...
  // koska users.splice palauttaa arrayn poistetuista arvoista ja tiedetään, että poistettiin vain yksi arvo, voidaan deletedUser ottaa palautetyn arrayn ensimmäisestä indeksistä "Array Destructuring"
